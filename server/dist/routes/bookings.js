...
router.get('/', authMiddleware_1.authenticateToken, async (req, res) => {
    ...
    try {
      ...
      `;
        ...
        ORDER BY b.created_at DESC
      `;
            const result = await db_1.default.query(query, [req.user.id]);
            res.json(result.rows);
        }
    }
    catch (err) {
...
router.get('/check-availability', async (req, res) => {
    ...
    try {
      ...
      LIMIT 1
    `;
        const result = await db_1.default.query(query, [room_id, check_out, check_in]);
...
router.post('/', authMiddleware_1.authenticateToken, async (req, res) => {
    ...
    try {
      ...
      RETURNING *
    `;
        const result = await db_1.default.query(query, [req.user.id, room_id, check_in, check_out, total_price]);
...
router.put('/:id', authMiddleware_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const checkRes = await db_1.default.query(checkQuery, [id]);
